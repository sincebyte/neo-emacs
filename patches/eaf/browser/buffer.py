...
class AppBuffer(BrowserBuffer):
    def __init__(self, buffer_id, url, arguments):
        ...
        if self.auto_import_chrome_cookies:
            ...
            #
            # Local fork patch: a cookie failure (e.g. macOS denying access to
            # Chrome's Cookies database, or a locked/sandboxed profile) must not
            # abort buffer creation.  If it raises, `create_buffer' never stores
            # the buffer in `buffer_dict', so the later `update_views' raises
            # KeyError, hides every view and the EAF window never opens.
            try:
                self.import_chrome_cookies(url)
            except Exception as e:
                print("[EAF] skip chrome cookie import: %s" % e)
