...
Note, we need hook this function to signal 'loadProgress', signal 'loadStarted' is not enough.'''
    ...
    def eventFilter(self, obj, event):
        ...
        if event.type() in [QEvent.Type.MouseButtonPress]:
            ...
            if event.button() == Qt.MouseButton.LeftButton and not self.buffer.input_mode:
                # A left click in the browser hands keyboard focus to it
                # (enable input mode), so Emacs loses focus and typing goes
                # to the page - same as the input-mode keybinding.
                self.buffer.current_event_string = "t"
                self.buffer.switch_to_input_mode()
        ...
        if event.type() == QEvent.Type.MouseButtonRelease:
            ...
            if platform.system() == "Darwin" and not self.buffer.input_mode:
...
class BrowserBuffer(Buffer):
    ...
    def switch_to_input_mode(self):
        # The event string (from `eval_function') selects the target state:
        # "t" -> enable, "nil" -> disable, otherwise toggle.  Enabling is
        # idempotent so a keypress always lands in input mode (focus in the
        # browser) instead of toggling an already-enabled mode off.
        if getattr(self, 'current_event_string', None) == "t":
            self.input_mode = True
        elif getattr(self, 'current_event_string', None) == "nil":
            self.input_mode = False
        else:
        ...
        if self.input_mode:
            ...
            if platform.system() == "Darwin":
                ...
                # The first activation can be overridden by Emacs finishing its
                # key event, and Qt's activateWindow only sticks once the app
                # activation has propagated. Retry a few times so that a single
                # press reliably hands keyboard focus to the browser window.
                for delay in (0, 100, 250, 500):
                    QTimer.singleShot(delay, self._focus_input_window)
        else:
    ...
    def _focus_input_window(self):
        """Give keyboard focus to the browser view window."""
        try:
            if not getattr(self, 'input_mode', False):
                return
            if platform.system() == "Darwin":
                tracker = QApplication.instance().macos_window_tracker
                tracker.bridge.activate_application(tracker.eaf_pid)
